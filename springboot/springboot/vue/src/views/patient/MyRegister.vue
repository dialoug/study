...
<!--    <div style="margin: 10px 0">
    ...
    <el-table :data="tableData" max-height="680">
      ...
      <el-table-column prop="priceState" label="缴费状态">
        <template v-slot="scope">
          <el-tag v-if="scope.row.priceState === '1'" type="success">已完成</el-tag>
          <!-- 如果挂号已完成且未缴费则提供缴费按钮 -->
          <el-button v-else-if="scope.row.state === '1' & scope.row.priceState === '0'" type="warning" @click="save(scope.row)">点击缴费</el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  methods: {
    load() {
      this.request.get("/registration/page", {
        ...
        this.total = res.data.total  // 记录总数
      })
    },
    reset() {
      this.name = ""
    ...
    save(row){
      this.form.id = row.id
      this.form.priceState = '1'
      this.request.post("/registration",this.form).then(res => {
        if (res.code === '200'){
          this.load()
